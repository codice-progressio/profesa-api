...
app.get(
  ...
  async (req, res) => {
    ...
    const total = await Color.aggregate([
      { $match },
      { $count: "total" },
    ]).exec()
...
app.put("/receta/centrifuga", (req, res, next) =>
{
  console.log(req.body)

  
  Color.findByIdAndUpdate(req.body._id, { $set: { receta: req.body } })
    .exec()
    .then(color => {
      if (!color) throw "No existe el color"
      delete req.body._id
      color.receta = { centrifuga: req.body }
